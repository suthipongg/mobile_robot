...
#define LOOPTIME 200                       //Looptime in millisecond
#define MILLI 0.001
const byte noCommLoopMax = 10;            //number of main loops the robot will execute without communication before stopping
unsigned int noCommLoops = 0;             //main loop without communication counter

const int PIN_ENCOD_Z_MOTOR_LEFT = 2;     //Z channel for encoder of left motor                    
const int PIN_ENCOD_Z_MOTOR_RIGHT = 3;    //Z channel for encoder of right motor        
...
const double encoder_cpr = 1;           //Encoder ticks or counts per rotation
const double gear_ratio = 100;
...
const int encodertreshould = 1; 
const int MAX_PWM_LEFT = 255;
const int MAX_PWM_RIGHT = 255;
...
void setup() {
  ...
  pinMode(PIN_ENCOD_Z_MOTOR_LEFT, INPUT_PULLUP);
  digitalWrite(PIN_ENCOD_Z_MOTOR_LEFT, HIGH);                // turn on pullup resistor
  attachInterrupt(digitalPinToInterrupt(PIN_ENCOD_Z_MOTOR_LEFT), encoderLeftMotor, RISING);

  // Define the rotary encoder for right motor
  pinMode(PIN_ENCOD_Z_MOTOR_RIGHT, INPUT_PULLUP);
  digitalWrite(PIN_ENCOD_Z_MOTOR_RIGHT, HIGH);                // turn on pullup resistor
  attachInterrupt(digitalPinToInterrupt(PIN_ENCOD_Z_MOTOR_RIGHT), encoderRightMotor, RISING);
}
